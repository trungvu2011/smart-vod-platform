  ...
  return (
    <div className="space-y-10 animate-slide-up max-w-5xl mx-auto">
      <div className="space-y-2">
        <h1 className="text-5xl font-black tracking-tighter text-wp-on-surface">Upload Video</h1>
        <p className="text-wp-on-surface-variant text-lg">Share knowledge with your organization.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3 space-y-8">
          {uploadState === 'idle' ? (
            ...
            >
              ...
              <div className="text-center space-y-1">
                <p className="text-wp-on-surface font-semibold">{dragActive ? 'Drop your video here' : 'Drag & drop your video file'}</p>
                <p className="text-xs text-wp-on-surface-variant">
                  or <span className="text-wp-primary font-medium">browse files</span>
                </p>
              </div>
              <p className="text-[11px] text-wp-outline">MP4, MOV, AVI, MKV - Max 5 GB</p>
                  ...
                  <div className="flex justify-between items-center text-[11px] font-medium text-wp-on-surface-variant">
                    <p>
                      {uploadState === 'uploading'
                        ? `Dang day len may... ${Math.min(Math.round(progress), 100)}%`
                        : `Worker dang xu ly... ${Math.min(Math.round(workerProgress), 100)}%`}
                    </p>
          ...
          <div className="bg-wp-surface-container-low rounded-3xl p-6 space-y-6">
            ...
            <div>
              <label className="block text-xs font-semibold text-wp-on-surface-variant mb-1.5 uppercase tracking-wide">Title</label>
              <input
                value={title}
            ...
            <div>
              <label className="block text-xs font-semibold text-wp-on-surface-variant mb-1.5 uppercase tracking-wide">Description</label>
              <textarea
                value={description}
            ...
            <div>
              <label className="block text-xs font-semibold text-wp-on-surface-variant mb-1.5 uppercase tracking-wide">Category</label>
              <select
                value={category}
