...
export default function AdminModerationPage() {
  ...
  const handleBulkApprove = async () => {
    try {
      await adminApi.bulkApproveVideos(Array.from(selected));
      showToast(`${selected.size} video(s) approved.`);
      fetchQueue();
    } catch (error) {
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto animate-fade-in">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {tabs.map(t => (
          <button key={t.key} onClick={() => { setTab(t.key); setPage1(); }}
      ...
      {selected.size > 0 && (
        <div className="glass-panel p-4 rounded-xl mb-6 flex items-center justify-between animate-fade-in">
          <span className="text-sm font-semibold text-wp-on-surface">
            <span className="text-wp-primary font-bold">{selected.size}</span> video(s) selected
          </span>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {videos.map((video) => (
              <div key={video.id} className={`group bg-wp-surface-container-low rounded-2xl overflow-hidden hover:scale-[1.01] transition-all duration-300 shadow-wp-card border ${selected.has(video.id) ? 'border-wp-primary/40 ring-1 ring-wp-primary/20' : 'border-wp-outline-variant/10'} flex flex-col`}>
                <div className="relative aspect-video overflow-hidden">
                  ...
                  {/* Status Badge */}
                  <div className={`absolute top-4 left-4 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase shadow-sm ${
                    video.status === 'PENDING' ? 'bg-amber-500/80 text-white' :
                    video.status === 'PROCESSING' ? 'bg-blue-500/80 text-white' :
                    video.status === 'READY' ? 'bg-emerald-500/80 text-white' :
                    'bg-red-500/80 text-white'
                  }`}>{video.status}</div>

                  {/* Checkbox */}
                  <div className="absolute top-4 right-4">
                    <input type="checkbox" checked={selected.has(video.id)}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      {video.creator?.avatarUrl ? (
                        <img src={video.creator.avatarUrl} alt="" className="w-5 h-5 rounded-full object-cover border border-wp-outline-variant/20" />
                      ) : (
                  ...
                  {(video.status === 'PENDING' || video.status === 'PROCESSING') && (
                    <div className="grid grid-cols-2 gap-3 mt-auto">
                      <button onClick={() => setApproveTarget(video)}
                        className="bg-wp-gradient text-wp-on-primary-fixed hover:text-wp-on-primary font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-md shadow-wp-primary/20"
                      >
                        <span className="material-symbols-outlined text-lg">check_circle</span>
                        Approve
                      </button>
                      <button onClick={() => setRejectTarget(video)}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to approve ${selected.size} video(s)? They will all be published immediately.`}
        confirmText={`Approve ${selected.size} Videos`}
        variant="primary"
      ...
      {bulkAction === 'reject' && (
        <RejectVideoModal
          open={true}
          videoTitle={`${selected.size} selected video(s)`}
...
// Helper to avoid lint warning about unused setPage
function setPage1() {}
